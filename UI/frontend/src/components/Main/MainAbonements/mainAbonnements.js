...
export default function MainAbonnements() {
    ...
    Resource.get("/abonements")
      .then((response) => {
        if (response.data.abonements?.abonementsWithServices?.length > 0) {
          ...
          const sortPrices = abonements
            .map((abonement) => abonement.abonement.price)
            .sort((a, b) => a - b);
          ...
          filterData(
            ...
            sortPrices[0],
            [],
            sortPrices[sortPrices.length - 1],
          );
          setSearchedAbonnements(
            sortAbonnements(abonements, sortFilter, sortOrder),
          );
        }
  ...
  function filterData(
    ...
    return data.filter((item) => {
      ...
      const matchesPrice =
        (minPrice === undefined || item.abonement.price >= minPrice) &&
        (maxPrice === undefined || item.abonement.price <= maxPrice);

      const containsAllValues = currentServices
        .map((column) => column.title)
        .every((value) =>
          item.services.map((column) => column.title).includes(value),
        );

      return (
        matchesName &&
        matchesPrice &&
        matchesValidityPeriod &&
        matchesVisitingTime &&
        containsAllValues
      );
    });
  ...
  function handleSortFilterChange(event) {
    ...
    setSearchedAbonnements(
      sortAbonnements(filtered, event.target.value, sortOrder),
    );
  }
