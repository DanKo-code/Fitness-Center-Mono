...
export default function AbonnementsModal({ onClose }) {
    ...
    try {
      if (currentAbonnement) {
        ...
        if (response.status === 200) {
          setAbonnements(
            abonnements.filter(
              (item) => item.abonement.id !== response.data.abonement.id,
            ),
          );
  ...
  const handleSubmit = async (event) => {
    ...
    if (currentAbonnement) {
      ...
      try {
        ...
        console.error(
          "Can't update abonement: " + JSON.stringify(error, null, 2),
        );
      }
    } else {
      //create abonement
      try {
        ...
        console.error(
          "Can't create abonement: " + JSON.stringify(error, null, 2),
        );
      }
    }
  };
  ...
  return (
    ...
    >
      <div>
        <IconButton
          onClick={handleCloseAbonnementsModal}
          size="large"
            ...
            {abonnements?.length > 0 ? (
              <div>
                {abonnements
                  .sort(
                    (a, b) =>
                      new Date(b.abonement.updated_time) -
                      new Date(a.abonement.updated_time),
                  )
                  .map((abonnement) => (
              ...
              <div>Нет никаких абонементов</div>
            )}
          </div>
        </div>
